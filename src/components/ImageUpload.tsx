...
export function ImageUpload({ value, onChange, label = "Upload Foto", compact }: Props) {
  ...
  return (
    <div className="space-y-2">
      {!compact && <label className="block text-xs font-medium text-[#5B5B7D] uppercase tracking-wider">{label}</label>}
      <div
        onClick={() => inputRef.current?.click()}
        className={`relative rounded-2xl text-center cursor-pointer transition-all duration-200 group ${compact ? "w-full h-full flex items-center justify-center" : "p-6"}`}
        style={{
          background: value
            ? "rgba(0,255,135,0.03)"
          ...
          ...(compact ? { minHeight: "100%", border: value ? "2px dashed rgba(0,255,135,0.3)" : "none" } : {}),
        }}
        onMouseEnter={(e) => {
      ...
      >
        {loading ? (
          ...
          <div className="relative inline-block">
            ...
            <div className="text-[#5B5B7D] text-center">
              <svg className="w-6 h-6 mx-auto opacity-30" fill="none" stroke="currentColor" viewBox="0 0 24 24">
